...
请求成功、失败可以拦截；响应成功、失败也可以拦截

---》所以有四个拦截

![image-20210613204741916](C:\Users\zyy\Desktop\Practical project Note\VueStudy\事件绑定.assets\image-20210613204741916.png)

axios.interceptors：：:全局拦截器

axios.interceptors .request.use({config=>console.log(config)}

,err=>{console.log(err)})	;//请求

注：拦截后必须要返回回去，否则最后拿不到这个config 

axios.interceptors.response：//响应

### 拦截器中都做什么呢？

![image-20210613204840732](C:\Users\zyy\Desktop\Practical project Note\VueStudy\事件绑定.assets\image-20210613204840732.png)
...
![image-20210613205251798](C:\Users\zyy\Desktop\Practical project Note\VueStudy\事件绑定.assets\image-20210613205251798.png)

2.请求时页面会出现等待的图标，就是在拦截器里面实现的show，当请求成功就不显示

3.有些页面必须要求登录了才访问-----》taken令牌，登录了再携带这个令牌发送请求
